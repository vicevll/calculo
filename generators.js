...
seF1:()=>{let x=R.int(-5,5),y=R.int(-5,5);let a=x+y,b=x-y;return{e:`$$\\begin{cases} x + y = ${a} \\\\ x - y = ${b} \\end{cases}$$`,s:[["Sumamos las ecuaciones: 2x = "+(a+b)+" ⇒ x = "+x+".",`x = ${x}`],["Restamos: 2y = "+(a-b)+" ⇒ y = "+y+".",`y = ${y}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seF2:()=>{let x=R.int(-5,5),y=R.int(-5,5),c=R.int(2,5);let a=x+y,b=c*x+y;return{e:`$$\\begin{cases} x + y = ${a} \\\\ ${c}x + y = ${b} \\end{cases}$$`,s:[["Restamos la 1ra de la 2da: "+(c-1)+"x = "+(b-a)+" ⇒ x = "+x+".",`x = ${x}`],["Sustituimos en la 1ra: "+x+" + y = "+a+" ⇒ y = "+y+".",`y = ${y}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seF3:()=>{let x=R.int(-5,5),y=R.int(-5,5),c=R.int(2,5);let a=c*x+y,b=x-y;return{e:`$$\\begin{cases} ${c}x + y = ${a} \\\\ x - y = ${b} \\end{cases}$$`,s:[["Sumamos: "+(c+1)+"x = "+(a+b)+" ⇒ x = "+x+".",`x = ${x}`],["Sustituimos: "+x+" - y = "+b+" ⇒ y = "+y+".",`y = ${y}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seF4:()=>{let x=R.int(-4,4),y=R.int(-4,4);let a=2*x+y,b=x+2*y;return{e:`$$\\begin{cases} 2x + y = ${a} \\\\ x + 2y = ${b} \\end{cases}$$`,s:[["De la 2da: x = "+(b)+" - 2y. Sustituimos en la 1ra.",`2(${b} - 2y) + y = ${a}`],["Resolviendo: "+(2*b)+" - 3y = "+a+" ⇒ y = "+y+".",`y = ${y}`],["Sustituimos: x = "+(b)+" - 2("+y+") = "+x+".",`x = ${x}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seF5:()=>{let x=R.int(-4,4),y=R.int(-4,4);let a=x+2*y,b=2*x-y;return{e:`$$\\begin{cases} x + 2y = ${a} \\\\ 2x - y = ${b} \\end{cases}$$`,s:[["Despejamos x de la 1ra: x = "+a+" - 2y.",`2(${a} - 2y) - y = ${b}`],["Resolviendo: "+(2*a)+" - 5y = "+b+" ⇒ y = "+y+".",`y = ${y}`],["Sustituimos: x = "+a+" - 2("+y+") = "+x+".",`x = ${x}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},

// --- SISTEMAS DE ECUACIONES 2x2 (DIFICIL) ---
seH1:()=>{let x=R.int(-5,5),y=R.int(-5,5),a=R.int(2,4),b=R.int(2,4),c=R.int(2,4),d=R.int(2,4);let p=a*x+b*y,q=c*x+d*y;return{e:`$$\\begin{cases} ${a}x + ${b}y = ${p} \\\\ ${c}x + ${d}y = ${q} \\end{cases}$$`,s:[["Multiplicamos 1ra por "+d+" y 2da por "+b+":",`${a*d}x + ${b*d}y = ${p*d} \\\\ ${c*b}x + ${b*d}y = ${q*b}`],["Restamos: "+(a*d-c*b)+"x = "+(p*d-q*b)+" ⇒ x = "+x+".",`x = ${x}`],["Sustituimos en 1ra: "+a+"("+x+") + "+b+"y = "+p+" ⇒ y = "+y+".",`y = ${y}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seH2:()=>{let x=R.int(-5,5),y=R.int(-5,5);let a=R.int(2,4),b=R.int(2,4),c=R.int(-4,-1),d=R.int(2,5);let p=a*x+b*y,q=c*x+d*y;return{e:`$$\\begin{cases} ${a}x + ${b}y = ${p} \\\\ ${c}x + ${d}y = ${q} \\end{cases}$$`,s:[["Eliminamos x: multiplicamos 1ra por "+(c)+" y 2da por "+a+".",`${a*c}x + ${b*c}y = ${p*c} \\\\ ${a*c}x + ${a*d}y = ${q*a}`],["Restamos: "+(b*c-a*d)+"y = "+(p*c-q*a)+" ⇒ y = "+y+".",`y = ${y}`],["Sustituimos: x = "+x+".",`x = ${x}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seH3:()=>{let x=R.int(-4,4),y=R.int(-4,4);let a=R.int(3,6),p=a*x+y;return{e:`$$\\begin{cases} ${a}x + y = ${p} \\\\ \\frac{x}{2} + \\frac{y}{3} = ${(x/2+y/3).toFixed(1)} \\end{cases}$$`,s:[["Multiplicamos la 2da por 6 para eliminar fracciones: 3x + 2y = "+(3*x+2*y)+".",`\\begin{cases} ${a}x + y = ${p} \\\\ 3x + 2y = ${3*x+2*y} \\end{cases}`],["Multiplicamos 1ra por 2: "+(2*a)+"x + 2y = "+(2*p)+". Restamos 2da.",(2*a-3)+"x = "+(2*p-3*x-2*y)+" ⇒ x = "+x+"."],["Sustituimos: y = "+y+".",`y = ${y}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seH4:()=>{let x=R.int(-4,4),y=R.int(-4,4);let a=R.int(2,5),b=R.int(2,5);let p=2*x+a*y,q=b*x-3*y;return{e:`$$\\begin{cases} 2x + ${a}y = ${p} \\\\ ${b}x - 3y = ${q} \\end{cases}$$`,s:[["Multiplicamos 1ra por 3 y 2da por "+(a)+":",`6x + ${3*a}y = ${3*p} \\\\ ${3*b}x - ${3*a}y = ${3*q}`],["Sumamos: "+(6+3*b)+"x = "+(3*p+3*q)+" ⇒ x = "+x+".",`x = ${x}`],["Sustituimos: y = "+y+".",`y = ${y}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
seH5:()=>{let x=R.int(-5,5),y=R.int(-5,5);let a=R.int(2,4),b=R.int(2,4),c=R.int(2,4),d=R.int(2,4);let p=a*x-b*y,q=c*x+d*y;return{e:`$$\\begin{cases} ${a}x - ${b}y = ${p} \\\\ ${c}x + ${d}y = ${q} \\end{cases}$$`,s:[["Multiplicamos 1ra por "+d+" y 2da por "+b+":",`${a*d}x - ${b*d}y = ${p*d} \\\\ ${c*b}x + ${b*d}y = ${q*b}`],["Sumamos: "+(a*d+c*b)+"x = "+(p*d+q*b)+" ⇒ x = "+x+".",`x = ${x}`],["Sustituimos: y = "+y+".",`y = ${y}`],["Solucion.",`x = ${x},\\; y = ${y}`]]}},
...
// --- FUNCION EXPONENCIAL (CEROS) ---
ex1:()=>{let a=R.int(2,8);return{e:`$$f(x) = e^{x} - ${a}$$`,s:[["Encuentra los ceros: f(x)=0 ⇒ e^x - "+a+" = 0.",`e^{x} = ${a}`],["Aplicamos ln a ambos lados.",`x = \\ln ${a}`]]}},
ex2:()=>{let a=R.int(2,8),b=R.int(2,4);return{e:`$$f(x) = ${b}e^{x} - ${a}$$`,s:[["Encuentra los ceros: "+b+"e^x - "+a+" = 0.",`e^{x} = ${a}/${b}`],["Aplicamos ln a ambos lados.",`x = \\ln\\left(\\frac{${a}}{${b}}\\right)`]]}},

// --- FUNCION LOGARITMICA (CEROS) ---
lg1:()=>{let a=R.int(2,6);return{e:`$$f(x) = \\ln x - ${a}$$`,s:[["Encuentra los ceros: ln x - "+a+" = 0.",`\\ln x = ${a}`],["Aplicamos exponencial: x = e^"+a+".",`x = e^{${a}}`]]}},
lg2:()=>{let a=R.int(2,5),b=R.int(2,5);return{e:`$$f(x) = ${a}\\ln x - ${b}$$`,s:[["Encuentra los ceros: "+a+"·ln x - "+b+" = 0.",`\\ln x = \\frac{${b}}{${a}}`],["Aplicamos exponencial.",`x = e^{${b}/${a}}`]]}},
...
function generatePool(tema, dificultad, count) {
  const map = {
    ...
    exponencial: { dificilito: 'ex' },
    logaritmica: { dificilito: 'lg' }
  };
